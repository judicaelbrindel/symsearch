...
def random_tree(max_depth=3, cur=0):
    ...
    const_keys = list(ALL_CONSTANTS.keys())
...
def symsearch(target, sigma, n=N_DEFAULT, depth=DEPTH, seed=SEED, verbose=True):
    """
    ...
    sigma : float
        The tolerance (1-sigma uncertainty or absolute tolerance).
    n : int
        Number of random expressions to generate (default: 1,000,000).
    depth : int
    ...
    -------
    dict with keys:
        n_valid       : number of valid expressions evaluated
        n_matches     : number of expressions within sigma
        p_empirical   : empirical probability of a match
        S_min         : minimum complexity among matches (S_INF if none)
        has_match     : True if at least one match found
        top_results   : list of top 10 matches (sigma, S, value, expression)
    """
...
def main():
    ...
    # Run search
    results = symsearch(target, sigma, n=n, verbose=True)
